# coding=utf-8
##########################################################################################
# Name: Owner_Missing_Policy_Bot
# Description:
# This is a monitoring script for cataloged data sources.  It
# checks if a data source has been assigned an owner after a specified
# period of time and if not trigger a notification to the steward assigned
...
# 1. Custom Field of type Object Set named Owners/Owner
#
...
policyBot = User.objects.get(username='')

# defaults for generated conversations
title='Action Required: Data Source missing an Owner'
text='This data source was created more than five days ago and has no Owner.  You are the assigned steward and are ' \
     'required to resolve ths issue. '

# default setting for duration before not having an owner on a data set is a violation
time_threshold = datetime.utcnow() - timedelta(days=5)
#print(str(time_threshold))

# check data sources without owners and finding stewards so a message can be trigger to them
...
for k in d:
    ...
    # get all generic custom fields which includes object sets and the one we are looking for Owner
    v=GenericFieldValue.objects.filter(otype=ObjectType.DATA).values()

    # declare variable so it can be used in the inner loop
    stewardid = 0

    # iterate over the custom fields looking to see if the Owner field is present in the data source
    for i in v:
        ...
        if i['oid']==cast_to_uuid(k['id']) and i['field_id'] == 8:
            ...
            # if the steward is a group we get the builtin name of the group
            # group
            if type == '38':
                steward = GroupProfile.objects.filter(id=id).values('builtin_name')
                #print('group:' + steward[0]['builtin_name'])

        # check if the custom field is Owner (field_id) 10028 is on the data source
        if i['oid']==cast_to_uuid(k['id']) and i['field_id'] == 10028:
            found = True

    # Continue if after iterating through all custom fields Owner is not found on data source and time threshold
    # surpassed

    if found == False and newt<time_threshold:
